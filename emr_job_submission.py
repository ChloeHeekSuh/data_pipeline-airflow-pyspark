...
SPARK_TEST_STEPS = [
    {
        ...
        'HadoopJarStep': {
            ...
            'Args': [
                ...
                '--executor-memory','3g',
                '--executor-cores','2',
                '--py-files','s3://mytest-wcd/job.zip',
                's3://mytest-wcd/workflow_entry.py',
                '-p', "{'input_path':'{{ task_instance.xcom_pull('parse_request', key='s3location') }}', 'name':'demo', 'file_type':'txt', 'output_path':'s3://weclouddata-data-engineer-bootcamp', 'partition_column':'job'}"
            ]
        }
    }
]
